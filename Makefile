...
volume:
	docker volume inspect pstore-src_postgres_data

pstore-db:
	docker-compose exec postgres-db psql --username makis --dbname=pstore_db
...
black-check:
	docker-compose exec api black --check --exlude=migrations .

black-diff:
	docker-compose exec api black --diff --exlude=migrations .
